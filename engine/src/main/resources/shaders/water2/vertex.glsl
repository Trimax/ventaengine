...
float getElevation(float x, float z) {
    ...
    for (float i = 0.0; i < wave.iterations; i++) {
        float noiseValue = snoise(p * frequency + timeElapsed * wave.speed);
        elevation += amplitude * noiseValue;
        amplitude *= wave.persistence;
        frequency *= wave.lacunarity;
    }
...
void main() {
    vec4 modelPosition = matrixModel * vec4(position, 1.0);
    ...
    gl_Position = matrixViewProjection * modelPosition;
}