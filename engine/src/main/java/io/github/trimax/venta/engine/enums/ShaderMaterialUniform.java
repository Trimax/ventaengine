...
public enum ShaderMaterialUniform {
    Metalness("metalness"),
    Roughness("roughness"),
    Opacity("opacity"),
    Tiling("tiling"),
    ...
    private static final Map<ShaderMaterialUniform, Map<Integer, String>> uniformNames = new HashMap<>();

    static {
        for (final ShaderMaterialUniform uniform : ShaderMaterialUniform.values()) {
            uniformNames.putIfAbsent(uniform, new HashMap<>());
            for (int materialID = 0; materialID < DefinitionsCommon.MAX_MATERIALS; materialID++)
                uniformNames.get(uniform).put(materialID, String.format("materials[%d].%s", materialID, uniform.uniformName));
        }
    }
}
