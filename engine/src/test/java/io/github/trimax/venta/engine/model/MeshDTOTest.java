...
public final class MeshDTOTest {
    ...
    void testGetVerticesArray_simpleTriangle() {
        ...
        assertEquals(COUNT_FLOATS_PER_VERTEX_MESH * triangle.vertices().size(), verticesArray.length, "The number of vertices must be correct");
        ...
        for (int i = 0; i < triangle.vertices().size(); i++) {
            assertEquals(0f, verticesArray[i * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_NORMAL_X], 1e-5, "Vertex " + i + " normal x must be correct");
            assertEquals(0f, verticesArray[i * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_NORMAL_Y], 1e-5, "Vertex " + i + " normal y must be correct");
            assertEquals(1f, verticesArray[i * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_NORMAL_Z], 1e-5, "Vertex " + i + " normal z must be correct");
        }

        /* Checking backed tangents */
        for (int i = 0; i < triangle.vertices().size(); i++)
            assertTrue(verticesArray[i * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_TANGENT_X] > 0, "Vertex " + i + " tangent direction should be correct");

        /* Checking backed bitangents */
        for (int i = 0; i < triangle.vertices().size(); i++)
            assertTrue(verticesArray[i * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_BITANGENT_Y] > 0, "Vertex " + i + " bitangent direction should be correct");
    ...
    private void verifyPosition(final int vertexID, final float[] verticesArray, final Vector3f expected) {
        verifyValue(vertexID, verticesArray[vertexID * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_POSITION_X], expected.x);
        verifyValue(vertexID, verticesArray[vertexID * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_POSITION_Y], expected.y);
        verifyValue(vertexID, verticesArray[vertexID * COUNT_FLOATS_PER_VERTEX_MESH + VERTEX_OFFSET_POSITION_Z], expected.z);
    }
