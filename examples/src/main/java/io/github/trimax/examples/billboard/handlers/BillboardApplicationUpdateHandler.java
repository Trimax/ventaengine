...
public final class BillboardApplicationUpdateHandler implements VentaEngineUpdateHandler {
    ...
    public void onUpdate(final Engine.VentaTime time, final VentaContext context) {
        ...
        state.getCamera().setPosition(new Vector3f((float) Math.sin(state.getCameraAngle()), 5f, (float) Math.cos(state.getCameraAngle())).mul(state.getCameraDistance()));
